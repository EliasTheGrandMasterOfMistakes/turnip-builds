...
EOF
	...
	meson build-android-aarch64 --cross-file "$workdir"/mesa-main/android-aarch64 -Dplatforms=android -Dgallium-drivers= -Dplatform-sdk-version=$sdkver -Dandroid-stub=true  -Dvulkan-drivers=freedreno -Dvulkan-beta=true -Dfreedreno-kmds=kgsl -Db_lto=true | tee "$workdir"/meson_log
